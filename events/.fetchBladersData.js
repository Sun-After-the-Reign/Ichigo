...
module.exports = {
  ...
  async run(bot, community, tournament) {
    ...
    for (let participant of participants.data) {
      ...
      let displayname = participant.attributes.name.includes(" | ") ? participant.attributes.name.replace(/\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Presentation}|\p{Emoji}\uFE0F/gu, '').split(" | ")[1] : participant.attributes.name.replace(/\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Presentation}|\p{Emoji}\uFE0F/gu, '')
